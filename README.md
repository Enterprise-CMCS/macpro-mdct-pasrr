...
[![CodeQL](https://github.com/Enterprise-CMCS/macpro-mdct-pasrr/actions/workflows/codeql-analysis.yml/badge.svg)](https://github.com/Enterprise-CMCS/macpro-mdct-pasrr/actions/workflows/codeql-analysis.yml)
[![Maintainability](https://qlty.sh/gh/Enterprise-CMCS/projects/macpro-mdct-pasrr/maintainability.svg)](https://qlty.sh/gh/Enterprise-CMCS/projects/macpro-mdct-pasrr)
[![Code Coverage](https://qlty.sh/gh/Enterprise-CMCS/projects/macpro-mdct-pasrr/coverage.svg)](https://qlty.sh/gh/Enterprise-CMCS/projects/macpro-mdct-pasrr)

### Integration Environment Deploy Status:

| Branch     | Build Status                                                                                                             |
| ---------- | ------------------------------------------------------------------------------------------------------------------------ |
| main       | ![deploy](https://github.com/Enterprise-CMCS/macpro-mdct-pasrr/actions/workflows/deploy.yml/badge.svg)                   |
| val        | ![deploy](https://github.com/Enterprise-CMCS/macpro-mdct-pasrr/actions/workflows/deploy.yml/badge.svg?branch=val)        |
| production | ![deploy](https://github.com/Enterprise-CMCS/macpro-mdct-pasrr/actions/workflows/deploy.yml/badge.svg?branch=production) |

PASRR is the CMCS MDCT application for collecting state data related to the Preadmission Screening and Resident Review (PASRR) program.
...
1. Clone the repo: `git clone https://github.com/Enterprise-CMCS/macpro-mdct-pasrr.git`
...
On a push to the repository or opening a pull request the [deploy.yml](https://github.com/Enterprise-CMCS/macpro-mdct-pasrr/blob/main/.github/workflows/deploy.yml) file runs. This script sets up and does a number of things. For a simple push it's mostly checking code coverage.
...
- Main (Pointed to [https://mdctpasrrdev.cms.gov/](https://mdctpasrrdev.cms.gov/)) is our development branch
- Val (Pointed to [https://mdctpasrrval.cms.gov/](https://mdctpasrrval.cms.gov/)) is our beta branch
- Production (Pointed to [http://mdctpasrr.cms.gov/](http://mdctpasrr.cms.gov/)) is our release branch

When a pull request is approved and merged into main the deploy script will spin up and upon completion will deploy to [https://mdctpasrrdev.cms.gov/](https://mdctpasrrdev.cms.gov/). If a user wants to deploy to val they can initiate the [Create Deployment PR action](https://github.com/Enterprise-CMCS/macpro-mdct-pasrr/actions/workflows/create-pr.yml) in GitHub actions, setting the target_branch variable to the destination environment. Once that pull request is approved, the deploy script will run again and upon completion will deploy to [https://mdctpasrrval.cms.gov/](https://mdctpasrrval.cms.gov/). So to quickly break it down:

- Submit pull request of your code to main
- Approve pull request and merge into main
- Deploy script runs and will deploy to [https://mdctpasrrdev.cms.gov/](https://mdctpasrrdev.cms.gov/)
- [Create Deployment PR action](https://github.com/Enterprise-CMCS/macpro-mdct-pasrr/actions/workflows/create-pr.yml) into val
- Approve pull request and **DO NOT SQUASH YOUR MERGE**, just merge it into val
- Deploy script runs and will deploy to [https://mdctpasrrval.cms.gov/](https://mdctpasrrval.cms.gov/)
- [Create Deployment PR action](https://github.com/Enterprise-CMCS/macpro-mdct-pasrr/actions/workflows/create-pr.yml) into production
- Approve pull request and **DO NOT SQUASH YOUR MERGE**, just merge it into production
- Deploy script runs and will deploy to [http://mdctpasrr.cms.gov/](http://mdctpasrr.cms.gov/).
...
PASRR pipes updates from the dynamodb tables to BigMac for downstream consumption. To add a topic, update the following locations:
