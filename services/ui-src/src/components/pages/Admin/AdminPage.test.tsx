...
import { BannerShape, BannerAreas } from "@pasrr/shared";
...
describe("<AdminPage />", () => {
  ...
  it("should render the current banners under subheaders", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "PASRR report dashboard", level: 3 })
    ).toBeVisible();
    ...
    expect(bannerPreviews[3]).toHaveTextContent("PASRR Alert");
    expect(bannerPreviews[3]).toHaveTextContent("mock description");
    expect(
      screen.getByRole("link", { name: "https://example.com/pasrr-alert" })
    ).toBeVisible();
  });
