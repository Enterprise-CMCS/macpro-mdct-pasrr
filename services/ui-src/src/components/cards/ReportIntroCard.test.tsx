...
const pasrrReportTypeCardComponent = (
  <RouterWrappedComponent>
    <ReportIntroCard title="PASRR">
...
describe("<ReportTypeCard />", () => {
  describe("Renders", () => {
    beforeEach(() => {
      render(pasrrReportTypeCardComponent);
    });

    test("PASRR ReportTypeCard is visible", () => {
      expect(screen.getByText("PASRR")).toBeVisible();
    });

    test("PASRR ReportTypeCard image is visible on desktop", () => {
      const imageAltText = "Spreadsheet icon";
      expect(screen.getByAltText(imageAltText)).toBeVisible();
    });
  });

  testA11y(pasrrReportTypeCardComponent);
});
