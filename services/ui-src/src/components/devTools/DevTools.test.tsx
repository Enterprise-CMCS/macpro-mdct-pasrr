...
import { ReportStatus, ReportType, PasrrSubType } from "@pasrr/shared";
...
describe("Test DevTools component", () => {
  ...
  it("Clicking DevTools button expands to more options", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Delete All PASRR Reports For PA" })
    ).toBeVisible();
  });
  ...
  it("Mock delete all report", async () => {
    ...
    const deleteAllBtn = screen.getByRole("button", {
      name: "Delete All PASRR Reports For PA",
    });
