...
describe("utils/auth", () => {
  describe("Test AuthManager Init", () => {
    test("Initializing when past expiration will require a new login", async () => {
      ...
      localStorage.setItem("mdctpasrr_session_exp", expired);

      initAuthManager();
      const clearedTime = localStorage.getItem("mdctpasrr_session_exp");
  ...
  describe("Test AuthManager", () => {
    ...
    test("updateTimeout", () => {
      ...
      const savedTime = localStorage.getItem("mdctpasrr_session_exp");
    ...
    test("getExpiration and refreshCredentials", async () => {
      ...
      localStorage.setItem("mdctpasrr_session_exp", initialExpiration);
    ...
    test("getExpiration returns an empty string if nothing is set", async () => {
      localStorage.removeItem("mdctpasrr_session_exp");
