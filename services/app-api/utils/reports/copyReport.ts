...
// TODO: populate with PASRR element/page ids that should not carry answers
// forward when a report is copied, once the PASRR report schema is defined.
const SKIP_COPY_ANSWER_IDS: string[] = [];

const SKIP_COPY_PAGE_IDS: string[] = [];

const copyAccordionAnswers = (
...
const copyAnswer = (
  ...
  _newSubType: PasrrSubType,
  status?: PageStatus
) => {
  for (const oldElement of oldElements) {
    // Copying over accordion group answers
    if ("accordions" in oldElement) {
      ...
      copyAccordionAnswers(oldElement.accordions, newElement.accordions);
    }
    ...
    if (newElement?.type === oldElement.type) {
      if (SKIP_COPY_ANSWER_IDS.includes(newElement.id)) {
        delete newElement.answer;
      } else {
...
export const copyAttachmentsAndTheirComments = async (newReport: Report) => {
  ...
  // Copy attachment-area uploads (including those nested in accordion groups)
  for (const page of pages) {
    if (!page.elements) continue;
    const elements = page.elements.flatMap((element) =>
      "accordions" in element
        ? element.accordions.flatMap((accordion) => accordion.elements)
        : [element]
    );
