...
import { ElementType, Report, ReportType } from "@pasrr/shared";
import {
  BatchWriteCommand,
...
describe("Report storage helpers", () => {
  ...
  describe("getReport", () => {
    test("should call DynamoDB to get report data", async () => {
      ...
      const report = await getReport(ReportType.PASRR, "CO", "mock-report-id");
  ...
  describe("deleteReport", () => {
    test("should call DynamoDB to delete report data", async () => {
      ...
      await deleteReport(ReportType.PASRR, "CO", "mock-report-id");
      ...
      expect(mockBatchWrite).toHaveBeenCalledWith(
        {
          RequestItems: {
            "local-reports": [
              {
                DeleteRequest: {
                  Key: { pKey: "PASRR#CO", sortKey: "mock-report-id" },
                },
              },
              {
                DeleteRequest: {
                  Key: { pKey: "PASRR#CO", sortKey: "mock-report-id#root" },
                },
              },
              {
                DeleteRequest: {
                  Key: { pKey: "PASRR#CO", sortKey: "mock-report-id#pageA" },
                },
              },
              {
                DeleteRequest: {
                  Key: { pKey: "PASRR#CO", sortKey: "mock-report-id#pageB" },
                },
              },
            ],
          },
  ...
  describe("queryReportsForState", () => {
    it("should call DynamoDB to get multiple reports", async () => {
      ...
      const reports = await queryReportsForState(ReportType.PASRR, "CO");
      ...
      expect(mockQuery).toHaveBeenCalledWith(
        {
          ...
          ExpressionAttributeValues: { ":pKey": "PASRR#CO" },
        },
        expect.any(Function)
      );
    });
