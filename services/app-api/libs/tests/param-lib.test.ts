...
describe("Path parameter parsing", () => {
  ...
  describe("parseReportTypeAndState", () => {
    test("should validate report type and state", () => {
      const event = {
        ...proxyEvent,
        pathParameters: { reportType: "PASRR", state: "CO" },
      };
      const result = parseReportTypeAndState(event)!;
      expect(result).toBeDefined();
      expect(result.reportType).toBe("PASRR");
      expect(result.state).toBe("CO");
    });
    ...
    test("should return false for invalid state", () => {
      const event = {
        ...proxyEvent,
        pathParameters: { reportType: "PASRR", state: "XX" },
      };
      const result = parseReportTypeAndState(event);
      expect(result).toBeUndefined();
    });
  ...
  describe("parseReportParameters", () => {
    test("should validate report type and state", () => {
      const event = {
        ...proxyEvent,
        pathParameters: { reportType: "PASRR", state: "CO", id: "foo" },
      };
    ...
    test("should return false for invalid state", () => {
      const event = {
        ...proxyEvent,
        pathParameters: { reportType: "PASRR", state: "XX", id: "foo" },
      };
      const result = parseReportParameters(event);
      expect(result).toBeUndefined();
    });
