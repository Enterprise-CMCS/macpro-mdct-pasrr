...
const getDynamoInfo: GetDynamoInfo = async (record) => {
  ...
  if (source.includes(`/${stage}-reports/`)) {
    ...
    const report = await getReport(ReportType.PASRR, payload.state, payload.id);
    return {
      topic: `${namespace}aws.mdct.pasrr.pasrr-reports.v0`,
      payload: report!,
    };
  } else if (source.includes(`/${stage}-comments/`)) {
    return {
      topic: `${namespace}aws.mdct.pasrr.pasrr-comments.v0`,
      payload,
    };
  } else {
    return;
