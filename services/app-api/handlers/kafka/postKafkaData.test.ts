...
import { ElementType, Report } from "@pasrr/shared";
...
describe("Kafka message sending", () => {
  ...
  it("should convert AWS Dynamo Stream Events to Kafka Messages", async () => {
    ...
    expect(mockSendBatch).toHaveBeenCalledWith({
      topicMessages: [
        {
          topic: "aws.mdct.pasrr.pasrr-reports.v0",
          messages: [
            {
              headers: {
                eventID: "eid-123",
  ...
  it("should group messages by topic", async () => {
    ...
    expect(mockSendBatch).toHaveBeenCalledWith({
      topicMessages: [
        {
          topic: "aws.mdct.pasrr.pasrr-reports.v0",
          messages: [expect.objectContaining({ key: "PASRR#CO#report123" })],
        },
        {
          topic: "aws.mdct.pasrr.pasrr-comments.v0",
          messages: [
  ...
  describe("when environment variables are not typical", () => {
    ...
    it("should respect topic namespace for dynamo events", async () => {
      process.env.topicNamespace = "--pasrr--my-branch--";
      const event = { Records: [mockReportEvent] };
      await handler(event);
      expect(mockSendBatch).toHaveBeenCalledWith(
        expect.objectContaining({
          topicMessages: [
            expect.objectContaining({
              topic: "--pasrr--my-branch--aws.mdct.pasrr.pasrr-reports.v0",
            }),
          ],
        })
      );
