...
import { ReportStatus, PasrrSubType, UserRoles } from "@pasrr/shared";
import { PasrrSubTypeTemplateMap } from "../../utils/constants";
...
const testEvent = {
  ...
  pathParameters: { reportType: "PASRR", state: "PA" },
...
describe("Test create report handler", () => {
  ...
  test("Test cannot create next report if not past start date", async () => {
    ...
    const date = new Date(PasrrSubTypeTemplateMap.A2.startDate - 1);
  ...
  test("Test successful create report after the first", async () => {
    ...
    const date = new Date(PasrrSubTypeTemplateMap.A2.openDate + 1);
    vi.setSystemTime(date);
    (queryReportsForState as Mock).mockReturnValueOnce([
      {
        id: "123",
