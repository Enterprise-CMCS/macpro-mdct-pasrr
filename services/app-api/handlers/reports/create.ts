...
import { error, PasrrSubTypeTemplateMap } from "../../utils/constants";
import { buildReport } from "../../utils/reports/buildReport";
import { putReport, queryReportsForState } from "../../storage/reports";
import { PasrrSubType, ReportOptions, isCompleteStatus } from "@pasrr/shared";
...
export const createReport = handler(
  ...
  async (request) => {
    ...
    if (reports.length > 0) {
      ...
      if (latestReport.subType === PasrrSubType.FINAL) {
        return badRequest(
      ...
      nextReportKey =
        PasrrSubTypeTemplateMap[latestReport.subTypeKey].nextReportSubType;
    }
    ...
    } = PasrrSubTypeTemplateMap[nextReportKey];
